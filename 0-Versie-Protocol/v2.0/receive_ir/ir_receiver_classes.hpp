   ...
   ///Get one bit from the bitstream
    ...
    bool get_bit(){
        if(IR_receiver_pin.get() == 0){
            ...
            hwlib::cout << "komt 1\n";
            while(!(IR_receiver_pin.get() == 0 || trace_time < 4000)){//wait until timeout time in stead of 1000
                hwlib::cout << "komt 2";
                trace_time = begin_time - hwlib::now_us();
            }
            hwlib::cout << "komt 3";
            if(trace_time >= 1000 && trace_time < 2000){
                return 1;
            }
            //else
            else {
    ...
    char16_t get_bits(){
        ...
        for(int i = 0; i < 15; i++){
            ...
            //i++;
            //add nowus to check how long has passed
        }
        return bitstream;
    }

...
private:
    ...
    void main(void){
        ...
        while(1){
            ...
            if(bitstream != -1){
                if(message_logic.decode(bitstream, player_id, weapon_id)){
                    if(player_id == 0){
                        play_time = (weapon_id*60);
                        hwlib::cout << ((int)weapon_id*60) << '\n';
                        break;
                    }
                    else{
                        continue;
        ...
        while(1){
            ...
            auto event = wait(button_pressed_flag + game_over_flag + received_information_channel);// + received_information_channel);
            //hwlib::cout << "komt hier1\n";
            if(event == game_over_flag){
                ...
                if(message_logic.decode(information, player_id, weapon_id)){
                    ...
                    //hwlib::cout << "pi: " << player_id << '\n';
                    //hwlib::cout << "wi: " << weapon_id << '\n';
                    ...
                    if(health > 0){
                        convert_int(health, health_array, 8,9);
                        health_array[10] = '\0';
                        lcd_struct.assignment(lcd_struct.line1, health_array);
                    }
                    else{
